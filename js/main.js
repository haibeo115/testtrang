$(document).ready(function() {
            ...
            .then(json => {
                ...
                fetch(location.origin + "/api/auth/getUser")
                    ...
                    .then(json_user => {
                        ...
                        if (json_user.data.tokenVerificationData !== undefined && viewAuthed !== undefined) return logoutUser();
                        if (json_user.success == false) return false;
                        if (json_user.data.tokenVerificationData !== undefined) return false;
                        loggedIn = true;
                        ...
                        utilsLoop = setInterval(function() {
                            fetch(location.origin + "/api/auth/getUser")
                                .then(response => response.json())
                                .then(json => {
                                    if (json.data.tokenVerificationData !== undefined) logoutUser();
                                });
                        }, 6000);
                    })
            });
    ...
    $('#openSidebar').click(() => {
        ...
        (!sidebarDiv.hasClass('main-nav-open')) ? sidebarDiv.addClass('main-nav-open'): sidebarDiv.removeClass('main-nav-open');
    })
